setTimeout(()=> {
	...
	`
		...
		if (allElements && spans) {
		    ...
		    const div = allElementsArray.find(e => e.textContent.trim() === 'Right to Work Documentation')
		    const container = div.closest('.FormHeader')
		    const element = container.querySelector('.SubformSummaryItem')
		    const rightToWorkButton = element.querySelector('.SummaryTableCell')
	        ...
	        var workPermitCheckedDate = ''
 		    ...
 		    setTimeout(() => {
		       //Get the upload date of the most recent photo
 		       ...
 		       const uploadedDatesArray = Array.from(container.querySelectorAll('.FileUploadListItemTimestamp'))
 		       var photoDates = []
 		       uploadedDatesArray.forEach((uploadedDate) => {
		            photoDates.push(uploadedDate.textContent.replaceAll('Uploaded on ', ''))
		        })

		        mostRecentPhotoDate = photoDates
		          .map((date) => {
